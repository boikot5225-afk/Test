...
reading_ready = "(()=>{const v=document.querySelector('#reader-reading-view');const root=document.querySelector('#reader-chapter-text');const r=root?.getBoundingClientRect();return !!(v&&getComputedStyle(v).display!=='none'&&root&&root.querySelectorAll('.reader-word').length>30&&r&&r.width>120&&r.height>120)})()"
end = time.time() + 35
...
if lang != 'fr':
    raise RuntimeError(f'Opened reader is not French: {lang!r}')
