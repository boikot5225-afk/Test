...
frozen = {
    ...
    # Второй раз: декодирование переведено на 16 кГц (контекст, в котором
    # вызывается decodeAudioData). Оно пересчитывает в частоту контекста, и
    # прежние 44.1/48 кГц держали час записи как ~1 ГБ Float32 без всякой
    # пользы — дальше конвейер всё равно рендерит в 16 кГц. Телефон этого не
    # переживал: распознавание падало на первом куске, ещё не начав грузить.
    # Третий раз: результат чистки транскрипта принимался без проверки, и
    # модель, пересказавшая кусок вместо правки, молча съедала содержание —
    # 54 минуты речи выходили книгой на 26 страниц. Теперь ответ короче 70%
    # исходника отбрасывается в пользу сырого текста.
    'js/reader-app.js': '040f1ddc192fde2122ab58173576ef3eb8ec0265dc63f85a4657bfbf37c2334f',
