...
open_replacement = '''  if (!book) { showToast('⚠️ Текст не найден'); return; }\n  readerCurrentBookId = id;\n\n  // Correct Chinese token boundaries are more important than a premature first\n  // paint. zh_dict_core.json is bundled inside the APK, so wait once before\n  // rendering instead of freezing a character-by-character fallback and then\n  // asking DeepSeek to translate the resulting debris.\n  if (readerCanonicalLang(readerBookLang(book)) === 'zh' && !readerZhCoreJson) {\n    try { await readerEnsureZhCoreJsonLoaded({ rerender: false }); }\n    catch (error) { console.warn('[reader zh] core-first paint fallback:', error?.message || error); }\n  }\n'''
...
replace_once(
    ...
    "import { createReaderChapterRenderer as createBaseRenderer } from './chapter-render-next.js?v=14';\n",
    "import { createReaderChapterRenderer as createBaseRenderer } from './chapter-render-next.js?v=15-zh-core-first';\n",
