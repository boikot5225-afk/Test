...
# whole functions structurally; nested braces inside their one-line bodies make
# a generic non-greedy regex brittle.  Stage tiny throwaway bodies here, then the
# immediately following toc122 patch replaces them with the real implementation.
# These stubs never ship in an APK.
...
# Make the exact boundaries expected by the next patch explicit.
for old, new in {
