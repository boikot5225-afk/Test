...
assert.doesNotMatch(interlinear, /text-overflow:\s*ellipsis/, 'interlinear Russian meaning must never be shortened visually');
assert.match(interlinear, /white-space:normal\s*!important/, 'long interlinear Russian meaning must wrap inside its word block');
assert.match(interlinear, /width:max-content\s*!important/, 'Unknown word block must expand to fit a readable meaning');
...
assert.equal(mod.compactGloss('свидетельствовать о происходящем'), 'свидетельствовать о происходящем', 'a full dictionary sense must not be cut by character count');
