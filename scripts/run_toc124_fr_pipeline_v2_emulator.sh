...
adb install -r "$APK"
adb push runtime-audit/fr-v2-toc124.epub /data/local/tmp/fr-v2-toc124.epub
adb shell run-as "$PKG" cp /data/local/tmp/fr-v2-toc124.epub cache/fr-v2-toc124.epub
adb shell am force-stop "$PKG"
adb shell am start -W \
  -a android.intent.action.VIEW \
  -d "file:///data/user/0/${PKG}/cache/fr-v2-toc124.epub" \
  -t application/epub+zip \
  -n "${PKG}/${ACT}" | tee runtime-audit/launch.txt
sleep 5
adb exec-out screencap -p > runtime-audit/toc124-00-import.png
