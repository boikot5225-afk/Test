...
if not cdp.eval("document.getElementById('main-app')?.style.display!=='none'"):
    ...
    if not clicked:
        raise RuntimeError('Guest button not found for toc136 Spanish parity audit')
...
result = cdp.eval(r"""(async()=>{
  ...
  root.dataset.readerBookId='toc136-es-parity-audit';
  root.dataset.renderedChapter='16';
  ...
  // The production word panel is lazy. Materialize the real owner so this audit
  // exercises the same Spanish Known/Unknown controls as a user tap without
  // asking readerOpenWordPanel to re-render the isolated fixture.
  ...
  try {
    ...
    await globalThis.readerSpanishPipelineV1RefreshNow('toc136-parity',true);
    ...
    if(typeof globalThis.readerSpanishGlossMode!=='function' || typeof globalThis.readerSetSpanishGlossMode!=='function') {
      throw new Error('Spanish gloss controls missing');
    }
    ...
    const enabledStyle={
      ...
      glossTop:getComputedStyle(gloss).top,
      paragraphTextAlign:getComputedStyle(word.closest('.reader-paragraph-text')).textAlign,
    };
    if(enabledStyle.wrapDisplay!=='inline' || enabledStyle.wrapPosition!=='relative' || enabledStyle.wordDisplay!=='inline' || enabledStyle.glossPosition!=='absolute' || enabledStyle.glossDisplay==='none') {
      throw new Error('toc136 Spanish inline annotation architecture inactive: '+JSON.stringify(enabledStyle));
    }
    ...
    await globalThis.readerSpanishPipelineV1RefreshNow('toc136-panel',true);
    await sleep(80);
    ...
    knownBtn.click();
    await sleep(80);
    await globalThis.readerSpanishPipelineV1RefreshNow('toc136-manual-known',true);
    ...
    title.textContent='cometerlos';
    await globalThis.readerSpanishPipelineV1RefreshNow('toc136-panel-again',true);
    await sleep(40);
    ...
    unknownBtn.click();
    await sleep(80);
    await globalThis.readerSpanishPipelineV1RefreshNow('toc136-manual-unknown',true);
    word=root.querySelector('[data-word="cometerlos"]');
...
if not result:
    raise RuntimeError('toc136 Spanish parity audit returned no result')
if result.get('cometer', {}).get('lemma') != 'cometer':
    raise RuntimeError('toc136 Spanish clitic lemma parity failed: ' + repr(result))
if result.get('bastaron', {}).get('lemma') != 'bastar':
    raise RuntimeError('toc136 Spanish conjugation lemma parity failed: ' + repr(result))
if result.get('enabledStyle', {}).get('wrapDisplay') != 'inline':
    raise RuntimeError('toc136 Spanish inline annotation architecture failed: ' + repr(result))
if result.get('disabledStyle', {}).get('glossDisplay') != 'none':
    raise RuntimeError('toc136 Spanish ordinary-text mode failed: ' + repr(result))
if not result.get('afterKnown', {}).get('known') or result.get('afterKnown', {}).get('wrapped'):
    raise RuntimeError('toc136 Spanish Known transition failed: ' + repr(result))
if not result.get('afterUnknown', {}).get('unknown') or not result.get('afterUnknown', {}).get('ru'):
    raise RuntimeError('toc136 Spanish Unknown transition failed: ' + repr(result))
