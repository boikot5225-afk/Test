...
adb install -r "$APK"
adb push runtime-audit/fr-context-toc125.epub /data/local/tmp/fr-context-toc125.epub
adb shell run-as "$PKG" cp /data/local/tmp/fr-context-toc125.epub cache/fr-context-toc125.epub
adb shell am force-stop "$PKG"
adb shell am start -W \
  -a android.intent.action.VIEW \
  -d "file:///data/user/0/${PKG}/cache/fr-context-toc125.epub" \
  -t application/epub+zip \
  -n "${PKG}/${ACT}" | tee runtime-audit/launch.txt
sleep 5
adb exec-out screencap -p > runtime-audit/toc125-00-import.png
PID="$(adb shell pidof "$PKG" | tr -d '\r')"
test -n "$PID"
adb forward tcp:9222 "localabstract:webview_devtools_remote_${PID}"
python3 scripts/bootstrap_toc123_french_reader_live.py
python3 scripts/audit_toc125_fr_context_batch_live.py
python3 scripts/audit_toc125_frozen_swipe.py
adb exec-out screencap -p > runtime-audit/toc125-99-final.png
