...
# 1) Shared readerAI path: prefer an authenticated Firebase client. A native
# install can have a valid REST-fallback session while the compat SDK's
# currentUser is null; the old code always chose the compat SDK and therefore
# sent an unauthenticated callable request.
...
reader_ai_new = '''function readerFirebaseClientWithAuthenticatedCallable() {
  ...
  // Keep the normal client as the error-reporting fallback when the user is
  // genuinely signed out; readerAI will then surface the real auth error.
...
# 2) Batch auth: inspect both Firebase clients instead of silently returning when
# the primary compat SDK has no currentUser but the REST fallback does.
...
# 3) Unknown targeting: a manually marked Unknown can briefly be rw-problem
# before the Migaku class bridge catches up. It still needs a missing gloss.
...
# 4) Cost control: do NOT spend DeepSeek on an Unknown that already has any
# usable Russian gloss from dictionary/ML Kit/context. Only blank slots qualify.
...
# Give local/native translation a small head start, then re-check immediately
# before spending a batch request. Items filled during this window are released
# from inFlight without being sent to DeepSeek.
replace_once(
...
# 5) Do not let one larger response run into the JSON ceiling. 16 tokens still
# gives a strong batching discount, while keeping responses reliably compact.
replace_once(
    'js/reader/zh-context-batch.js',
...
# Bust runtime paths after toc114/toc113 patches.
replace_once(
