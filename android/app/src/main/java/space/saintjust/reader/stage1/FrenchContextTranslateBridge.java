...
/**
 * toc124: French -> Russian context refinement for the French reader.
 *
 * The web layer always paints a local dictionary gloss first. This bridge is
 * only a background second opinion for an exact occurrence, so model download
 * or inference can never hold first paint, pagination or gestures hostage.
 */
public final class FrenchContextTranslateBridge {
    ...
    private void enqueue(Request request) {
        if (closed) {
            sendFailure(request.id, "FR→RU контекстный переводчик остановлен");
            return;
        }
        if (queue.size() >= MAX_QUEUE) {
            sendFailure(request.id, "Очередь FR→RU переполнена");
            return;
        }
    ...
    private void sendSuccess(String requestId, String source, String translated) {
        ...
        try {
            ...
            payload.put("provider", "mlkit_fr_ru_occurrence");
