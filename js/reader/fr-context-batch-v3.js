// toc125 — paragraph-level contextual French gloss refinement.
//
// The primary French pipeline owns layout and paints an immediate local gloss.
// This module NEVER inserts wrappers, NEVER observes DOM mutations and NEVER
// paints a loading placeholder. It sends all visible Unknown occurrences from
// one paragraph to DeepSeek in one batch and only replaces existing gloss text
// after a contextual answer arrives. Reader navigation remains independent.

const SENSES_URL = new URL('../../../frreader/fr_ru_senses.json?v=3', import.meta.url).href;
const CACHE_KEY_BASE = 'an2_reader_fr_context_batch_v3';
...
const state = globalThis.__readerFrContextBatchV3 || {
  cache: null,
  senses: null,
  sensesPromise: null,
  inFlight: new Set(),
...
globalThis.__readerFrContextBatchV3 = state;
...
function paragraphTargets(paragraph, context, dict) {
  ...
  words.forEach((el, tokenIndex) => {
    ...
    const { wrap, node } = glossNode(el);
    if (!wrap || !node) return;
    // Never expose an old loading placeholder from toc124 if a stale DOM/cache
    // survives a hot reload; the local pipeline owns the immediate fallback.
...
function withTimeout(promise, ms) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('frContextBatch timeout')), ms);
...
async function callBatch(context, targets) {
  ...
  const fn = firebase.app().functions(functionRegion()).httpsCallable('frContextBatch');
  ...
  const result = await withTimeout(fn({ context, targets: payloadTargets }), CALL_TIMEOUT_MS);
...
function cacheResult(target, result) {
  ...
  if (!ru || confidence < 0.5) return false;
  const cache = loadCache();
...
async function refine(reason = 'event') {
  ...
  for (const paragraph of paragraphs) {
    ...
    // Intentionally do not await all paragraphs serially: start at most two
    // paragraph batches while the already-painted local glosses remain usable.
...
if (typeof window !== 'undefined' && !window.__readerFrContextBatchV3Bound) {
  window.__readerFrContextBatchV3Bound = true;
...
export { normalize, cleanRu, paragraphContext, refine };
