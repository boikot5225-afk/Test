...
export function createReaderChapterRenderer({
  ...
  function render() {
    ...
    if (canonicalLang(activeReaderLang) === 'zh' && needsZhCoreLoad()) {
      ensureZhCoreLoaded({ rerender: true });
    }
    ...
    if (chapterText) {
      ...
      if (book.format === 'song' && chapter?.songSection) {
        ...
        chapterText.innerHTML = paragraphs.map((paragraph, index) => {
          ...
          return `
      <div class="reader-paragraph ${index === paragraphIndex ? 'active' : ''}" data-p="${index}">
        <div class="reader-paragraph-text">${renderParagraphText(paragraph, index)}</div>
        ${index === paragraphIndex && translation && !getTranslationsHidden() ? renderTranslationBlock(translation) : ''}
        ${index === paragraphIndex && book.readerAnalyses?.[translationKey] && !getTranslationsHidden() ? renderAnalysisBlock(book.readerAnalyses[translationKey]) : ''}
