...
const CACHE_VERSION = 'v71.0-language-shell';
...
const CORE_ASSETS = [
  ...
  'js/lang-separation.js', 'js/lang-core.js', 'js/lang-reader.js',
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(caches.open(CACHE_NAME).then(async (cache) => {
    await Promise.allSettled(CORE_ASSETS.map(async (asset) => {
      ...
      catch (error) { console.warn('[sw] precache skipped:', asset, error); }
    }));
  }));
});
...
self.addEventListener('activate', (event) => {
  event.waitUntil(caches.keys().then((keys) => Promise.all(
    keys.filter((key) => key.startsWith('an2-cache-') && key !== CACHE_NAME).map((key) => caches.delete(key))
