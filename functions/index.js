...
CONTEXT: ${body.context || ''}`;
    ...
    if (lang === 'en') {
      return `You are an English-Russian lexical assistant for a language reader. Analyze the selected English token IN ITS CONTEXT. Return ONLY valid JSON with keys: pos (noun|verb|adjective|adverb|preposition|pronoun|other), lemma, ru, level (A1|A2|B1|B2), form_note, note. Rules: lemma is the base/infinitive form; ru must be the meaning of the token AS USED IN THIS SPECIFIC CONTEXT, not the most common dictionary sense — if the token is part of an idiom or phrasal verb (e.g. "road" in "get the show on the road", "up" in "give up"), ru gives the contextual meaning and note names the idiom/phrasal verb with its Russian meaning (e.g. "get the show on the road — начать, приступить к делу"); form_note briefly explains the form if it is inflected (e.g. "past tense", "plural", "3rd person singular"). No gender needed.
